...
const TextEditor = (
  ...
  useEffect(() => {
     ...
     * @type {EditorState}
     */
    const newEditorState = EditorState.createWithContent(rawContentState, decorator)
    dispatch({ type: "setEditorState", payload: newEditorState })
  }, [defaultValue, reset])

  const color = "#000000"
  const onURLChange = e => dispatch({ type: "setURLValue", payload: e.target.value })
  /**
  ...
  const promptForLink = e => {
    ...
    if (!selection.isCollapsed()) {
      ...
      if (linkKey) {
        // //remove link
        dispatch({ type: "setEditorState", payload: RichUtils.toggleLink(editorState, selection, null) })
  ...
  const confirmLink = e => {
    ...
    dispatch({
      type: "setEditorState", payload: RichUtils.toggleLink(
        newEditorState,
  ...
  return (
    <>
      ...
      <div className="text-editor-default"
      ...
      >
        <UrlInputModal
          isOpen={showURLInput}
          handleCloseModal={() => dispatch({ type: "setShowURLInput", payload: false })}
        ...
        <div className="text-editor-default-controls">
          <button type="button" onClick={() => onBoldClick()}><EditorB color={color}/></button>
          <button type="button" onClick={() => onUnderlineClick()}><EditorU color={color}/></button>
          <button type="button" onClick={() => onItalicClick()}><EditorI color={color}/></button>
          <button type="button" onClick={promptForLink}><EditorLink color={color}/></button>
          <button type="button"><EditorFile color={color}/></button>
        </div>
        <Editor
